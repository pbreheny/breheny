tpower <- function(n,delta,lam=c(1,-1),b=c(delta,0),sig=1,alpha=.05,w=rep(1,g),n1,n2)
  {
    g <- length(b)
    ...
    for (i in 1:nn)
      {
        ...
        C <- qt(1-alpha/2,df[i])
        ncp <- abs(crossprod(lam,b)/(sig*sqrt(qd(lam,X))))
        power[i] <- 1-pt(C,df[i],ncp)+pt(-C,df[i],ncp)
      }
    return(power)
  }
