binom.bayes <- function(x, n, a=1, b=1, ci=.95, plot=FALSE, add=FALSE, ...)
{
  ...
  l.h <- uniroot(f,c(0,qbeta(1-ci,a+x,n-x+b))-1e-7)$root
  ##l.h <- uniroot(f,c(0,1))$root
  u.h <- qbeta(pbeta(l.h,a+x,n-x+b)+ci,a+x,n-x+b)
  cat(100*ci,"% HPD interval: (",l.h,", ",u.h,")\n",sep="")
  if (plot | add) {
    xx <- seq(0,1,len=399)
    if (add) {
      lines(xx, dbeta(xx,a+x,n-x+b), lwd=3, ...)
    } else {
